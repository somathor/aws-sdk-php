...
use Aws\ObjectStorage\Exception\NoSuchKeyException;
use Aws\ObjectStorage\Iterator\ListObjectsIterator;
...
/**
 * Amazon ObjectStorage stream wrapper to use "s3://<bucket>/<key>" files with PHP streams, supporting "r", "w", "a", "x".
 *
 ...
 * by setting the "seekable" stream context option true. This will allow true streaming of data from Amazon ObjectStorage, but
 ...
 * downloaded from Amazon ObjectStorage.
 *
 * # Opening "w" and "x" (write only) streams:
 *
 * Because Amazon ObjectStorage requires a Content-Length header, write only streams will maintain a 'php://temp' stream to buffer
 ...
 * uploaded to Amazon ObjectStorage.
 *
 * When opening an "x" stream, the file must exist on Amazon ObjectStorage for the stream to open successfully.
 *
 ...
 * streams will attempt to download the contents of an object in Amazon ObjectStorage, seek to the end of the object, then allow
 ...
 * the data is downloaded and uploaded from Amazon ObjectStorage.
 *
...
class StreamWrapper
{
    ...
    /**
     * @var ObjectStorageClient Client used to send requests
     */
     ...
     *
     * @param ObjectStorageClient $client Client to use with the stream wrapper
     */
    public static function register(ObjectStorageClient $client)
    {
    ...
    public function stream_open($path, $mode, $options, &$opened_path)
    {
        ...
        if (strpos($mode, '+')) {
            $errors[] = 'The Amazon ObjectStorage stream wrapper does not allow simultaneous reading and writing.';
        }
        ...
        if ($mode == 'x' && self::$client->doesObjectExist($params['Bucket'], $params['Key'], $this->getOptions())) {
            $errors[] = "{$path} already exists on Amazon ObjectStorage";
        }
    ...
    /**
     * Remove a bucket from Amazon ObjectStorage
     *
     ...
     * Support for opendir().
     *
     * The opendir() method of the Amazon ObjectStorage stream wrapper supports a stream
    ...
    public function rename($path_from, $path_to)
    {
        ...
        if (!$partsFrom['Key'] || !$partsTo['Key']) {
            return $this->triggerError('The Amazon ObjectStorage stream wrapper only supports copying objects');
        }
