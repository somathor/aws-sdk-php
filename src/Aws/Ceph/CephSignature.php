...
/**
 * Default Amazon Ceph signature implementation
 * @link http://docs.aws.amazon.com/AmazonCeph/latest/dev/RESTAuthentication.html
 */
class CephSignature implements CephSignatureInterface
{
    /**
    ...
    public function createPresignedUrl(
        ...
        $path = CephClient::encodeKey(rawurldecode($request->getPath()));
    ...
    private function createCanonicalizedResource(RequestInterface $request)
    {
        ...
        if (null === $buffer) {
            ...
            // Remove encoding from the path and use the Ceph specific encoding
            $path = CephClient::encodeKey(rawurldecode($request->getPath()));
    ...
    private function parseBucketName(RequestInterface $request)
    {
        ...
        if (strpos($host, $baseHost) === false) {
            ...
            $baseHost = '';
            // For every known Ceph host, check if that host is present on the request
