...
use Aws\Ceph\Exception\NoSuchKeyException;
use Aws\Ceph\Iterator\ListObjectsIterator;
...
/**
 * Amazon Ceph stream wrapper to use "s3://<bucket>/<key>" files with PHP streams, supporting "r", "w", "a", "x".
 *
 ...
 * by setting the "seekable" stream context option true. This will allow true streaming of data from Amazon Ceph, but
 ...
 * downloaded from Amazon Ceph.
 *
 * # Opening "w" and "x" (write only) streams:
 *
 * Because Amazon Ceph requires a Content-Length header, write only streams will maintain a 'php://temp' stream to buffer
 ...
 * uploaded to Amazon Ceph.
 *
 * When opening an "x" stream, the file must exist on Amazon Ceph for the stream to open successfully.
 *
 ...
 * streams will attempt to download the contents of an object in Amazon Ceph, seek to the end of the object, then allow
 ...
 * the data is downloaded and uploaded from Amazon Ceph.
 *
...
class StreamWrapper
{
    ...
    /**
     * @var CephClient Client used to send requests
     */
     ...
     *
     * @param CephClient $client Client to use with the stream wrapper
     */
    public static function register(CephClient $client)
    {
    ...
    public function stream_open($path, $mode, $options, &$opened_path)
    {
        ...
        if (strpos($mode, '+')) {
            $errors[] = 'The Amazon Ceph stream wrapper does not allow simultaneous reading and writing.';
        }
        ...
        if ($mode == 'x' && self::$client->doesObjectExist($params['Bucket'], $params['Key'], $this->getOptions())) {
            $errors[] = "{$path} already exists on Amazon Ceph";
        }
    ...
    /**
     * Remove a bucket from Amazon Ceph
     *
     ...
     * Support for opendir().
     *
     * The opendir() method of the Amazon Ceph stream wrapper supports a stream
    ...
    public function rename($path_from, $path_to)
    {
        ...
        if (!$partsFrom['Key'] || !$partsTo['Key']) {
            return $this->triggerError('The Amazon Ceph stream wrapper only supports copying objects');
        }
