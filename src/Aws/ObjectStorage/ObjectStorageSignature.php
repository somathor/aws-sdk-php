...
/**
 * Default Amazon ObjectStorage signature implementation
 * @link http://docs.aws.amazon.com/AmazonObjectStorage/latest/dev/RESTAuthentication.html
 */
class ObjectStorageSignature implements ObjectStorageSignatureInterface
{
    /**
    ...
    public function createPresignedUrl(
        ...
        $path = ObjectStorageClient::encodeKey(rawurldecode($request->getPath()));
    ...
    private function createCanonicalizedResource(RequestInterface $request)
    {
        ...
        if (null === $buffer) {
            ...
            // Remove encoding from the path and use the ObjectStorage specific encoding
            $path = ObjectStorageClient::encodeKey(rawurldecode($request->getPath()));
    ...
    private function parseBucketName(RequestInterface $request)
    {
        ...
        if (strpos($host, $baseHost) === false) {
            ...
            $baseHost = '';
            // For every known ObjectStorage host, check if that host is present on the request
